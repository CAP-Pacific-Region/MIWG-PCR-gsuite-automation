...
| `TENANT_REGION_CAPWATCH_DATA_FOLDER_ID` | `addOutOfWingDutyPositions_()` in `UpdateMembers.gs`; the signature web app | **Optional.** A region-wide CAPWATCH extract, read-only, for duties this tenant's own pull cannot see. CAPWATCH scopes an extract to the echelon downloaded, so a wing pull has **no row** for a member's REGION or NATIONAL billet. Points at the PCR tenant's folder, **shared cross-tenant to this tenant's automation account** — a manual grant that fails silently if it lapses. Never widens the roster and never feeds duty-group matching. Blank = feature off. See [Signature Web App §7](SIGNATURE_WEB_APP.md). |
...
| `TENANT_2SV_SETUP_GROUP` | `getTenantConfig_()` → `twoSvGroupEmail_()` in `TwoSvSetupGroup.gs` | The 2SV setup group — membership exempts a member from 2SV enforcement while they enroll. `pruneTwoSvSetupGroup()` removes a member the night they enroll, or once their 7-day window is up. **Blank disables the prune**, and blank is the default: the module will not guess at a group whose membership is a security exemption. **The help-desk app is a separate script project** and reads the same address from its own `WEBAPP_2SV_SETUP_GROUP` — set both, to the same group ([Admin Web App §6](ADMIN_WEB_APP.md)). |
...
| 7 | `addSecondaryDomainAliases()` | Daily, 7–8 AM | Give accounts on the `Secondary Aliases` tab a matching address on the tenant's secondary domain. **Seniors only** (`TENANT_SECONDARY_EMAIL_DOMAIN`). Must run *after* `updateAllMembers()` so a same-morning account already exists. On a new alias, auto-configures Gmail *Send mail as* for the member (service-account impersonation) and emails them (how to switch addresses in Compose + CAPR 120-1 §6.9 official-use reminder); opt out of the email with `SECONDARY_ALIAS_NOTIFY=false`. |
...
| 9 | `pruneTwoSvSetupGroup()` | Daily, 8–9 AM | Cross-reference the 2SV setup group against 2SV enrollment and remove members who enrolled, or whose 7-day exemption expired — whichever comes first. Blank `TENANT_2SV_SETUP_GROUP` disables it. Run `previewTwoSvSetupGroup()` once before arming. |
| 10 | `syncMemberCalendarsDaily()` | Daily | Share unit calendars with new/transferred members. |
| 11 | `updateChatSpaces()` | Daily or weekly | Sync squadron/committee Chat spaces & membership. |
| 12 | `updateResources()` | Weekly (Sun) | Aircraft/vehicle Calendar resources + squadron buildings. **Seniors only** (`MANAGE_RESOURCES`). |
| 13 | `manageLicenseLifecycle()` | Monthly | Reactivate renewed, delete long-ineligible accounts to free seats. |
| 14 | `notifyLSCodeChanges()` | Weekly (Mon) | Mail unit CCs when a member's FBI background check is granted or lapses. **Seniors only.** The cadence *is* the reported detection window. |
| 15 | `notifyRecoveryEmailCompliance()` | Monthly (1st) | Mail unit CCs + personnel officers + deputies about members whose email record blocks a password reset, whose account lacks 2-Step Verification, or whose account was never signed into (60+ days). **Seniors + cadets.** |
| 16 | `notifyUnwelcomedAccounts()` | Monthly (1st) | Mail **IT only** about accounts created since the ledger baseline with no welcome email recorded — i.e. created in the Admin console or by GAM, which never sends one. Silent when there is nothing to report. Requires `seedWelcomeLedger(false)` to have been run once first, or it no-ops. |
...
- `getMembers()`, `getSquadrons()`, `getAEMembers()` — data builders (used by tests/other modules).
  `getMembers()` also merges **out-of-wing duty positions** when
  `TENANT_REGION_CAPWATCH_DATA_FOLDER_ID` is set: a member's region or national billet is absent
...
- Audit / cleanup prep (read-only, or writes a review sheet only): `..._auditReceiveListPosting()`
  ...
  a sender on the other tenant (both a person writing directly, and cross-tenant fan-out from a
