...
| **Never changes group membership** | The duties it adds go into `dutyPositions` — what signatures read — and **not** into `dutyPositionIds` / `dutyPositionIdsAndLevel`, which is what `UpdateGroups.gs` matches duty-based groups on. A wing member's region billet must not quietly add them to a wing duty group. |
