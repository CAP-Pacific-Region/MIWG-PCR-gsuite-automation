 ...
 * Version: 1.1.0
 * Date: 2026-08-19
 * Changes: 1.1.0 — Add now runs the WHOLE process immediately, not just the alias:
 *   on a genuine new alias it also configures Gmail "Send mail as" and emails the
 *   member (Notify.gs). This has to happen here because an alias this page creates
 *   reads as 'already present' by the time the nightly run fires, so the nightly
 *   notifier would skip it. Both steps run outside the sheet lock and are
 *   best-effort. Response gains sendAsConfigured / welcomeSent.
...
function apiAddByCapid(capid, primaryEmail) {
  ...
  try {
    const sheet = webappGetAliasSheet_();
    existing = webappFindRowByEmail_(sheet, account.email);
    ...
    result = account.aliases.indexOf(alias) !== -1
  ...
  // The whole point of doing this here rather than leaving it to the nightly run:
  // an alias this page just created reads as 'already present' by the time the
  // trigger fires, so the trigger would configure Send-As and email NObody. We do
  // it now, at the click. Gate on a genuine fresh insert (status begins 'ADDED —'):
  // a reconcile or an already-present alias must not re-notify.
  //
  // Runs OUTSIDE the lock — it makes no sheet writes, and impersonation + Gmail
  // calls should not hold up another admin's add. Best-effort: webappConfigureSendAs_
  // and webappMaybeSendWelcome_ never throw, so a mail hiccup cannot undo the alias
  // or fail the request.
  let sendAsOutcome = 'not-attempted';
  let welcomeSent = false;
  if (result.status.indexOf('ADDED') === 0) {
    sendAsOutcome = webappConfigureSendAs_(account.email, alias, account.name, account.isAdmin);
    welcomeSent = webappMaybeSendWelcome_(account.email, alias, account.name, sendAsOutcome);
  }
  ...
  return {
    ...
    sendAsConfigured: webappSendAsReady_(sendAsOutcome),
