...
- **Restored account no longer stays suspended for an extra run** (`src/accounts-and-groups/UpdateMembers.gs`) - the `Users.update` that un-suspends a just-undeleted account (added in the soft-delete restore fix) could 404 while Google was still propagating the `Users.undelete`, and the shared retry helper doesn't retry 404s. New `updateAfterUndelete_()` retries specifically on that error with backoff, so a restored account is un-suspended in the same run instead of the next one.
...
- **Calendar Resources Module** (`src/calendar-resources/`) - New optional module syncing CAPWATCH equipment and squadron locations to Google Workspace Calendar resources
  * Aircraft as bookable Calendar resources, grouped under airport buildings resolved via the FAA Aviation Weather Center API
  * Vehicles as bookable Calendar resources under squadron buildings
  * All wing squadrons added as Calendar buildings regardless of vehicle assignment, so members can select their unit's meeting location when creating events
  * Maintenance Officer contact info attached to aircraft resources; Transportation Officer contact info attached to vehicle resources
  * Airport name/coordinate cache (`AirportCache.json`) to minimize AWC API calls on subsequent runs
  * `AirportOverrides.json` — wing-configurable overrides for FAA LIDs the AWC API doesn't resolve
  * `AircraftOverrides.json` — wing-configurable overrides for aircraft with placeholder airport IDs in CAPWATCH (e.g. `ASSIGN`)
  * `previewAircraftResources()` and `previewVehicleResources()` dry-run functions
  * Full module documentation at `src/calendar-resources/README.md`

### Changed

- **README.md** — Updated with Calendar Resources module in overview, architecture diagram, installation steps, trigger table, module documentation, and troubleshooting entries. Version bumped to 2.1.0.
...
**Breaking Changes**: None — the Calendar Resources module is entirely optional. Existing modules are unaffected.

**New Requirements** (only if using Calendar Resources):

1. Enable **Admin SDK Directory API** in your Apps Script project under Services
2. Upload `AirportOverrides.json` and `AircraftOverrides.json` to your CAPWATCH data folder in Drive
3. Add a weekly trigger for `syncResources()`

**Steps**:

1. Copy `src/calendar-resources/UpdateResources.gs` into your Apps Script project
2. Enable Admin SDK Directory API under Services
3. Upload both JSON override files to your CAPWATCH Drive folder (edit as needed for your wing's aircraft and airports)
4. Run a test sync manually and review execution logs
5. Add a weekly time-driven trigger for `syncResources()` (Sunday 6:00 AM recommended)

**Benefits**:

- Aircraft and vehicles bookable directly in Google Calendar
- All squadron locations available when creating events
- Maintenance and Transportation Officer contacts surfaced in Calendar resource details
...
- Check the [Troubleshooting Guide](https://github.com/cap-miwg/gsuite-automation/blob/master/docs/TROUBLESHOOTING.md)
