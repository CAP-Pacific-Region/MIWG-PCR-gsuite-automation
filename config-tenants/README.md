...
- `region.json` — Pacific Region (complete; verified from the live "PCR Automation" `config.gs`)
...
2. Either enter the `TENANT_*` keys by hand in **Project Settings → Script Properties**, or paste
   ...
   values are already filled in — paste `setup-region.gs` and run `setupRegionScriptProperties()`.
